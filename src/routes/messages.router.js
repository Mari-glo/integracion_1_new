...
router.get('/', async (req, res) => { 
    try {
        //let messages = await messageServer.find();
...
router.post('/', async (req, res) => {
    ...
    let result = await messageServer.create({ user, message });
...
router.put('/:id_message ', async (req, res) => {
    ...
    let result = await messageServer.updateOne({ _id: id_message  }, messagesToReplace);
    res.send({ result: "Exito", payload: result });
});
...
router.delete('/:id_message ', async (req, res) => {
    let { id_message  } = req.params;
    let result = await messageServer.deleteOne({ _id: id_message });
    res.send({ result: "Exito", payload: result });
});
